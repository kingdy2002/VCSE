...
class Agent(common.Module):
    ...
    def train(self, data, state=None):
        metrics = {}
        state, outputs, mets = self.wm.train(data, self._task_behavior.actor,self._task_behavior._extr_target_critic, state)
        metrics.update(mets)
        start = outputs["post"]
        reward = lambda seq: self.wm.heads["reward"](seq["feat"]).mode()
        int_reward = lambda seq: self.wm.heads["int_reward"](seq["feat"]).mode()
        metrics.update(
            self._task_behavior.train(self.wm, start, data["is_terminal"], reward, int_reward)
        )
...
class WorldModel(common.Module):
    def __init__(self, config, obs_space, act_space, tfstep):
        ...
        self.layer_norm = common.nets.NormLayer('keras')
        self.layer_norm_state = common.nets.NormLayer('keras')
        self.layer_norm_value = common.nets.NormLayer('keras')

        self.random_projection_matrix = tf.Variable(
            tf.random.normal(
                (
                    self.config.rssm.deter + config.rssm.stoch * config.rssm.discrete,
                    self.config.reduce_dim,
                ),
                mean=0.0,
                stddev=1.0/ self.config.reduce_dim,
    ...
    def train(self, data, actor,critic, state=None):
        with tf.GradientTape() as model_tape:
            model_loss, state, outputs, metrics = self.loss(
                data, actor,critic, state, training=True
            )
        modules = [
            self.rssm,
            self.wm_vit_encoder,
    ...
    def loss(self, data, actor, critic, state=None, training=False):
        ...
        tmp = tf.stop_gradient(self.rssm.get_feat(post))
        latant_state = tf.matmul(tmp, self.random_projection_matrix)
        dist = critic(tmp)
        value = dist.mean()
        value =  tf.stop_gradient(value) # B * T
        latant_state = latant_state.reshape([B *  T,-1])
        _, C = latant_state.shape
        value = value.reshape([B * T])
        value = self.layer_norm_value(value)
        value = tf.cast(value,tf.float32)

        if self.config.use_int_reward:
            if self.config.use_vcse :
                value = value.reshape([B * T, 1])
                value = tf.cast(value,tf.float32)
                latant_state = tf.cast(latant_state,tf.float32)
                latant_state = tf.cast(latant_state,tf.float32)
                sim_matrix_s = tf.norm(
                    latant_state[:, None, :] - latant_state[None, :, :], axis=-1
                )
                # k-nn particle entropy estimator
                sim_matrix_v = tf.norm(
                    value[:, None, :] - value[None, :, :], axis=-1
                )
                # B* T , k => 
                sim_matrix = tf.math.reduce_max(tf.concat([tf.expand_dims(sim_matrix_s,-1),tf.expand_dims(sim_matrix_v,-1)],-1),axis=-1,keepdims=False)
                eps = (
                    -tf.math.top_k(-sim_matrix, k=self.config.k , sorted=True).values[:, -1]
                )
                state_norm = (
                    -tf.math.top_k(-sim_matrix_s, k=self.config.k , sorted=True).values[:, -1]
                )
                value_norm = (
                    -tf.math.top_k(-sim_matrix_v, k=self.config.k , sorted=True).values[:, -1]
                )
                eps = eps.reshape([-1, 1])
                sim_matrix_v = sim_matrix_v < eps
                sim_matrix_v = tf.cast(sim_matrix_v,dtype=tf.float32)
                sim_matrix_s = sim_matrix_s < eps
                sim_matrix_s = tf.cast(sim_matrix_s,dtype=tf.float32)
                n_v = tf.math.reduce_sum(sim_matrix_v,axis=1,keepdims = True)
                reward = tf.math.digamma(n_v+1) / C + tf.math.log(eps * 2 + 0.0001)
                data["int_reward"] = tf.cast(
                    tf.reshape(tf.stop_gradient(reward), [B, T]),
                    data["reward"].dtype,
                )
                metrics["int_reward"] = reward.mean()
                ...
                reward = tf.math.log(1+state_norm)
                data["int_reward"] = tf.cast(
                    tf.reshape(tf.stop_gradient(reward), [B, T]),
                    data["reward"].dtype,
                )
...
class ActorCritic(common.Module):
    def __init__(self, config, act_space, tfstep):
        ...
        self.extr_critic = common.MLP([], **self.config.critic)
        if self.config.slow_target:
            self._target_critic = common.MLP([], **self.config.critic)
            self._extr_target_critic = common.MLP([], **self.config.critic)
            self._updates = tf.Variable(0, tf.int64)
        else:
            self._target_critic = self.critic
            self._extr_target_critic = self.extr_critic
        ...
        self.extr_critic_opt = common.Optimizer(
            "extr_critic", **self.config.extr_critic_opt
        )
        self.ext_rewnorm = common.StreamNorm(**self.config.reward_norm)
        if self.config.use_vcse : 
            self.int_rewnorm = common.StreamNorm(**self.config.int_reward_norm_vcse)
        else :
            self.int_rewnorm = common.StreamNorm(**self.config.int_reward_norm_se)
        
    def train(self, world_model, start, is_terminal, reward_fn, int_reward_fn):
        metrics = {}
        ...
        with tf.GradientTape() as actor_tape:
            ...
            seq["reward"] = reward
            b = self.config.beta_init
            if self.config.use_int_reward :
                int_reward = int_reward_fn(seq)
                if self.config.use_int_stream_norm : 
                    int_reward, int_mets1 = self.int_rewnorm(int_reward)
                    int_mets1 = {f"int_reward_{k}": v for k, v in int_mets1.items()}
                seq["int_reward"] = b * int_reward
                metrics["beta"] = b
                target, mets2 = self.target(seq,int=True)
                extr_target, int_mets2 = self.target(seq, extr_critic=True)
            else :
        ...
        if self.config.use_int_reward :
            with tf.GradientTape() as extr_critic_tape:
                extr_critic_loss, int_mets4 = self.critic_loss(seq, extr_target, extr=True)
            metrics.update(
                self.extr_critic_opt(extr_critic_tape, extr_critic_loss, self.extr_critic)
            )
        ...
        if self.config.use_int_reward :
            if self.config.use_int_stream_norm : 
                metrics.update(**int_mets1, **int_mets2, **int_mets4)
            else :
                metrics.update(**int_mets2, **int_mets4)
    ...
    def critic_loss(self, seq, target, extr=False):
        # States:     [z0]  [z1]  [z2]   z3
        # Rewards:    [r0]  [r1]  [r2]   r3
        # Values:     [v0]  [v1]  [v2]   v3
        # Weights:    [ 1]  [w1]  [w2]   w3
        ...
        # Loss:        l0    l1    l2
        if extr:
            critic = self.extr_critic
            metrics_label = "extr_critic"
        else:
            critic = self.critic
            metrics_label = "critic"
        if extr:
            tmp = tf.stop_gradient(seq["feat"][:-1])
            dist = critic(tmp)
        else :
            dist = critic(seq["feat"][:-1])
        target = tf.stop_gradient(target)
        weight = tf.stop_gradient(seq["weight"])
        critic_loss = -(dist.log_prob(target) * weight[:-1]).mean()
        metrics = {metrics_label: dist.mode().mean()}
        return critic_loss, metrics

    def target(self, seq, int=False , extr_critic=False):
        # States:     [z0]  [z1]  [z2]  [z3]
        # Rewards:    [r0]  [r1]  [r2]   r3
        # Values:     [v0]  [v1]  [v2]  [v3]
        ...
        if extr_critic:
            metrics_label = "extr_critic"
            target_critic = self._extr_target_critic
        else:
            metrics_label = "critic"
            target_critic = self._target_critic
        
        if int:
            reward = tf.cast(seq['reward'], tf.float32) + tf.cast(seq['int_reward'], tf.float32)
        else :
            reward = tf.cast(seq['reward'], tf.float32)
        disc = tf.cast(seq["discount"], tf.float32)
        value = target_critic(seq["feat"]).mode()
        ...
        metrics = {}
        metrics[f"{metrics_label}_slow"] = value.mean()
        metrics[f"{metrics_label}_target"] = target.mean()
        return target, metrics

    def update_slow_target(self):
        for critic, target_critic in zip(
            [self.critic, self.extr_critic],
            [self._target_critic, self._extr_target_critic],
        ):
            if self.config.slow_target:
